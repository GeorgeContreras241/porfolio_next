"use client";
import { Animation } from "../organisms/Animation";
...
export const Projects = () => {
  ...
  return (
    <section
      id="proyectos"
      className="relative flex flex-col z-50 py-20 px-4 max-w-[1280px] w-full m-auto"
    >
      <Animation>
      ...
      <button
        className="text-white border border-[#00cfff] px-6 py-2 mb-4 self-end hover:cursor-pointer hover:bg-[#00cfff]/20"
      ...
      <Animation>
        {design ? (
          <div className="flex flex-col gap-4 md:flex-row">
              ...
              {tabs.map((title) => (
                <button
                  ...
                  className={`py-2 w-full px-3 rounded-xl transition-all border border-transparent hover:border-cyan-500 hover:text-cyan-400 ${
                    title === info
                      ? "bg-cyan-700/20 text-cyan-300 font-semibold border-cyan-500"
                      : ""
                  }`}
                >
                  {title}
            ...
            <div className="md:w-3/4 border border-[#17569580] rounded-xl backdrop-blur-md p-6 shadow-neon-blue min-h-[350px]">
                  ...
                  <div className="text-white w-full max-w-xl">
                    ...
                    <ul className="flex flex-wrap gap-2 text-xs mb-4">
                      {data[0]?.technologies.map((tech, index) => (
                        <li
                          key={index}
                          className="bg-cyan-800/60 px-3 py-1 rounded-full text-white"
                        >
                          {tech}
                        </li>
                      ))}
          ...
          <div className="text-white grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 min-h-[520px]">
            {projects.map((data, index) => (
              <article
                key={index}
                className="border flex flex-col border-[#17569580] min-h-[500px]"
              >
                <div>
                  <img src={data.img} alt="Proyecto 1" />
                </div>
                <div className="flex flex-col  flex-1 py-4 px-4 backdrop-blur-3xl">
                  <h4 className="text-xl sm:text-2xl font-bold text-cyan-400 mb-2">
                    {data.title}
                  </h4>
                  <p className="text-slate-300 text-sm mb-4 ">
                  ...
                  <div className="flex flex-row justify-end mt-auto gap-2">
                    <Link data={data.url} text={"Visitar"} />
                    <Link
                      data={data.gitHub}
                      git={data.featured}
                      text={
